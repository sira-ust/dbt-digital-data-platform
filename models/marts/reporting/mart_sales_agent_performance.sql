-- mart_sales_agent_performance — daily sales-rep scorecard from system event log.
-- Grain: activity_date (UTC) x username x source_code.
--
-- Phase 1: log-only (MySQL system events). Order value metrics come from
-- send-order payload on successes. Phase 2 can enrich with NAV/replication.
...
with sales_events as (
    ...
    where actor_type = 'sales'
      ...
      and source_code in ('PDA-A', 'CatalogFS-I', 'CatalogFS-A')

),

orders as (

    select * from {{ ref('fct_orders_submitted') }}
    where actor_type = 'sales'
      and username is not null
      and source_code in ('PDA-A', 'CatalogFS-I', 'CatalogFS-A')
